...
public class CellposeBuilder {
    ...
    protected CellposeBuilder(File builderFile) {
        ...
        try {
            ...
            if (doLog) logger.info("Builder parameters loaded from {}", builderFile);
    ...
    public Cellpose2D build() {
        ...
        if (file.exists()) {
            if (doLog) logger.info("Provided model '{}' is a file. Assuming custom model", file);
        }
        ...
        if (this.overlap != null) {
            ...
            if (this.cellposeParameters.containsKey("diameter")) { // Diameter was set
                ...
                if (diameter == 0.0) {
                    ...
                    if(doLog) logger.info("Tile overlap was not set and diameter was set to {}. Will default to {} pixels overlap. Use `.setOverlap( int )` to modify overlap", diameter, cellpose.overlap);
                } else {
                }
                cellpose.overlap = (int) (diameter * 2);
                if(doLog) logger.info("Tile overlap was not set, but diameter exists. Using provided diameter {} x 2: {} pixels overlap", diameter, cellpose.overlap);
            } else { // Nothing was set, let's get lucky
                cellpose.overlap = 30 * 2;
                if(doLog) logger.info("Neither diameter nor overlap provided. Overlap defaulting to {} pixels. Use `.setOverlap( int )` to modify overlap", cellpose.overlap);
            }
        }
        if(doLog) logger.info("If tiling is necessary, {} pixels overlap will be taken between tiles", cellpose.overlap);
        ...
        if (saveBuilder) {
            ...
            try {
                ...
                fw.flush();
                fw.close();
                if(doLog) logger.info("Cellpose Builder serialized and saved to {}", savePath);
